 ...
 * Define file paths
 */
let shpFilePath = path.join(__dirname, '../_data/map.shp');
...
function getMesaCityProjection() {
    return new Promise((resolve, reject) => {
        ...
        let prjFile = path.join(__dirname, '../_data/map.prj');
...
function getZones(preprocessed) {
    return new Promise((resolve, reject) => {
        ...
        let filePath = path.join(__dirname, `../_data/map${preprocessed}.dbf`);
 ...
 * @returns {Promise<any>}
 */
//TODO: call getZones and extracts customers by features with properties matching a customer
// props: ['UNIQUEID', 'NUMBER_OF_', ... 'UNIT_BUILD', 'LATITUDE', 'LONGITUDE', 'X_COORDINA', 'Y_COORDINA', 'DAY', 'ROUTE'];
